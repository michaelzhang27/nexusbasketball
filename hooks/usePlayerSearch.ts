...
function buildParams(
  ...
  if (filters.conferences.length)
    p.set("conferences", filters.conferences.join(","));
  if (filters.classYears.length)
    p.set("class_years", filters.classYears.join(","));
  if (filters.portalStatuses.length)
    p.set("portal_statuses", filters.portalStatuses.join(","));
...
export function usePlayerSearch(
  ...
  type CommittedState = {
    ...
    searchQuery: string;
    sortField: SortField;
    dataView: string; // or 'mens' | 'womens' if you want stricter typing
  };

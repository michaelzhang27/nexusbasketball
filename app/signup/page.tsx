...
const FEATURES = [
  { num: '01', label: 'Portal Tracking',    desc: 'Every D1 player in the transfer portal with live stats & contact status.' },
  { num: '02', label: 'Custom Fit Scoring', desc: 'Per-40 weighted models by position. Your coefficients, not a generic algo.' },
  { num: '03', label: 'Roster Simulation',  desc: 'Build depth charts and stress-test your rotation before committing to an offer.' },
  { num: '04', label: 'NIL Modeling',       desc: 'Roster-adjusted NIL ranges based on position, usage, and your remaining budget.' },
]

const PARTICLES = [
  { label: 'FIT 91/99',  x:  6, y: 20, dur: 22, delay:  0,   opacity: 0.40 },
  ...
  { label: 'REB% 14.2',  x: 58, y: 48, dur: 20, delay: 10.5, opacity: 0.30 },
  { label: '+12 P/M',    x: 42, y: 25, dur: 26, delay:  4.7, opacity: 0.33 },
  { label: 'WIN% 72',    x: 55, y: 65, dur: 19, delay:  8.1, opacity: 0.28 },
  { label: 'PACE 71.4',  x: 30, y: 42, dur: 22, delay:  1.5, opacity: 0.30 },
]
...
export default function SignupPage() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/teams`)
      ...
      .catch(() => {/* backend unavailable — team dropdown stays empty */})
  }, [])
        ...
        <div className="hidden lg:flex flex-col flex-1 relative overflow-hidden">
          ...
          <div
            ...
            style={{
              ...
              WebkitMaskImage: 'radial-gradient(ellipse 80% 80% at 40% 50%, black 20%, transparent 100%)',
            }}
          />
          ...
          <div className="left-content relative z-10 flex flex-col justify-between h-full px-12 py-10">
            ...
            <div className="space-y-5 max-w-[400px]">
              <div className="inline-flex items-center gap-2 bg-blue-950/60 border border-blue-800/40 rounded-full px-4 py-1.5 backdrop-blur-sm">
                <span className="w-1.5 h-1.5 rounded-full bg-blue-400" style={{ animation: 'blink-dot 2.5s ease infinite' }} />
                <span className="text-xs text-blue-300 font-medium tracking-wide">Get started in seconds</span>
              </div>

              <h2 className="text-3xl font-bold leading-tight tracking-tight text-white">
                Your recruiting edge<br />
              ...
              <div className="space-y-2 pt-2">
                {FEATURES.map((f, i) => (
                  <div
                    ...
                    style={{
                      background: activeFeature === i ? 'rgba(59,130,246,0.07)' : 'transparent',
                      borderLeft: activeFeature === i ? '2px solid rgba(59,130,246,0.5)' : '2px solid transparent',
                    }}
                    onMouseEnter={() => setActiveFeature(i)}
                  >
                    <div className="flex items-center gap-2 mt-0.5 shrink-0">
                      <span className="text-[10px] font-bold text-gray-700 tabular-nums">{f.num}</span>
                      <div
                        className="w-4 h-4 rounded-full flex items-center justify-center transition-all duration-300"
                        style={{
                          background: activeFeature === i ? 'rgba(59,130,246,0.2)' : 'transparent',
                          border: activeFeature === i ? '1px solid rgba(59,130,246,0.4)' : '1px solid rgba(255,255,255,0.08)',
                        }}
                      >
                        {activeFeature === i && <Check className="w-2.5 h-2.5 text-blue-400" />}
                      </div>
                    </div>
                    <div>
                      <p className={cn(
                        'text-sm font-semibold transition-colors duration-300',
                        activeFeature === i ? 'text-blue-300' : 'text-gray-400'
                      )}>
                        {f.label}
                      </p>
                      <p className={cn(
                        'text-[11px] leading-relaxed transition-all duration-300 overflow-hidden',
                        activeFeature === i ? 'text-gray-500 max-h-10 mt-0.5' : 'text-transparent max-h-0'
                      )}>
                        {f.desc}
                      </p>
                    </div>
            ...
            <p className="text-[11px] text-gray-700">© 2025 Nexus Analytics · NCAA D1 Men's Basketball</p>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center justify-center w-full lg:w-[520px] lg:flex-shrink-0 px-8 py-12 relative overflow-y-auto">
          ...
          <div className="lg:hidden flex items-center gap-2 mb-10">
            <Link href="/" className="flex items-center gap-2">
              <span className="font-bold tracking-widest text-blue-400 text-sm uppercase">Nexus Analytics</span>
              <Badge variant="amber" className="text-[10px] px-1.5 py-0.5">BETA</Badge>
            </Link>
          </div>
          ...
          <div className="relative z-10 w-full max-w-sm">
            ...
            >
              ...
              <p className="text-sm text-gray-500 mt-1.5">Get started with Nexus Analytics.</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              >
                <label className="text-xs font-medium text-gray-400 tracking-wide uppercase">Full name</label>
              ...
              >
                <label className="text-xs font-medium text-gray-400 tracking-wide uppercase">Password</label>
                <div className="relative">
                  <input
                ...
                {password.length > 0 && (
                  <div className="space-y-1 pt-0.5">
                        ...
                        <div
                          ...
                          style={{
                            background: n <= pwStrength.score
                              ? (pwStrength.color.replace('bg-', '').includes('red')
                                  ? '#ef4444'
                                  : pwStrength.color.replace('bg-', '').includes('amber')
                                  ? '#f59e0b'
                                  : pwStrength.color.replace('bg-', '').includes('emerald')
                                  ? '#10b981'
                                  : '#3b82f6')
                    ...
                    <p className={cn(
                      ...
                      : pwStrength.score <= 3 ? 'text-blue-400'
                      : 'text-emerald-400'
                    )}>
                      {pwStrength.label}
                    </p>
              ...
              >
                <div>
                  <label className="text-xs font-medium text-gray-400 tracking-wide uppercase">Your Team</label>
                  <p className="text-[11px] text-gray-600 mt-1">Select the program you coach. Used for conference record predictions.</p>
                </div>
                <div className="relative">
                  <button
                    type="button"
                  ...
                  >
                    <span className="truncate">{teamName || 'Search for your team…'}</span>
                    <ChevronDown className={cn('w-4 h-4 text-gray-600 shrink-0 ml-2 transition-transform', teamDropdownOpen && 'rotate-180')} />
                  </button>
                      ...
                      <div className="max-h-48 overflow-y-auto">
                        {teams
                          .filter(t =>
                            ...
                            t.conference.toLowerCase().includes(teamSearch.toLowerCase())
                          )
                          .slice(0, 80)
                          .map(t => (
                            <button
                              ...
                              className={cn(
                                ...
                                teamName === t.team ? 'text-blue-300 bg-blue-900/15' : 'text-gray-300'
                              )}
                            >
                              <span>{t.team}</span>
                              <span className="text-gray-600 text-[10px] ml-2 shrink-0">{t.conference}</span>
                            </button>
                          ))
                        }
                        {teams.filter(t =>
                          ...
                          <p className="text-xs text-gray-600 text-center py-4">No teams found</p>
                        )}
                      </div>
                    </div>
                  )}
              ...
              >
                <div>
                  <label className="text-xs font-medium text-gray-400 tracking-wide uppercase">Data view</label>
                  <p className="text-[11px] text-gray-600 mt-1">Choose which transfer portal you&apos;d like to work with.</p>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  {([
                    { value: 'mens'   as DataView, label: "Men's",   sub: 'D1 portal' },
                    { value: 'womens' as DataView, label: "Women's", sub: 'Coming soon' },
                  ]).map(({ value, label, sub }) => (
                    <button
                      key={value}
                      type="button"
